...
results_path <- "results_20160814/PCLDA/Runs/vs2/"
...
results_from_vs <- function(x, pi, k=NULL, data_set = NULL, iter = c(19000,20000)){
  ...
  for(i in seq_along(index)){ # i <- 1
    ...
    plot_df_tmp <- plot_df_tmp[plot_df_tmp$iter >= iter[1] & plot_df_tmp$iter <= iter[2],]
    if(i == 1) {plot_df <- plot_df_tmp} else {plot_df <- rbind(plot_df,plot_df_tmp)}
  }
  mean(plot_df$phiDensity)
}
...
lik_plt_vs <- function(x, data_set, k, seed=NULL, it =c(100,20000), pis = NULL){
  df <- parse_vs_paths_to_dataframe(paths = x$paths)
  
  part_df <- df[df$data_full == data_set & df$k == k, ]
  if(!is.null(seed))  part_df <- part_df[part_df$seed == seed,]
  if(!is.null(pis))  part_df <- part_df[part_df$pi %in% pis,]
  for(i in seq_along(pis)){
    part_df$pi[part_df$pi == pis[i]] <- names(pis)[i]
  }
  index <- part_df$no
  ...
  plt <- ggplot(data = plot_df[plot_df$iter <= it[2] & plot_df$iter >= it[1], ]) + 
    ...
    geom_line() + guides(color=guide_legend(title=expression(pi)))
...
for (pi in pis){ # pi <- pis[1]
  for(k in ks){ # k <- ks[1]
    full_vs_plts <- list() 
    for(data_set in data_sets){ # i <- 3
      file_name <- paste0(paste("vs_res", pi, k , data_set, sep = "-"))
      print(file_name)
      #all_plots[[length(all_plots)+1]] <- plt <- full_plt_vs(x = res_lik, pi=pi, k = k, y=y[i], it=y_it[[i]])+ xlab("Iteration") + ylab(y_ylab[i])  
      all_results[[length(all_results)+1]] <- results_from_vs(x = res_lik, pi=pi, k = k, data_set = data_set, c(19000, 20000))
      names(all_results)[length(all_results)] <- file_name
    }
  }
}
...
for (data_set in data_sets){ # data_set <- "pubmed"
  ...
  for(i in seq_along(ks)){ # i <- 1
    file_name <- paste0(paste("vs_lik_plot", data_set, ks[i], sep = "-"),".png")
    lik_vs_plts[[i]] <- lik_plt_vs(x = res_lik, data_set = data_set, k = ks[i], pis = pis)
    all_plots[[length(all_plots)+1]] <- lik_vs_plts[[i]] <- lik_vs_plts[[i]]  +  #my_theme() +
...
save(all_plots, all_results, file = paste0(output_folder, "vs_results.Rdata"))
